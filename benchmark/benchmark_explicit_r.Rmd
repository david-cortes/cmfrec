...
The timings are measured on a Ryzen 7 2700 CPU (3.2Ghz, 8c/16t), using OpenBLAS 0.3.13.
...
microbenchmark({
    ...
    model <- CMF(X_full, k=50, niter=15, lambda=0.05, scale_lam=TRUE,
                 use_cg=TRUE, finalize_chol=FALSE,
                 user_bias=FALSE, item_bias=FALSE, verbose=FALSE)
}, times=1L)
...
microbenchmark({
    suppressWarnings({
        ...
        X_full_scaled <- biScale(X_full_si, maxit=5,
                                 ...
                                 col.scale=FALSE,
                                 trace=FALSE)
        model <- softImpute(X_full_scaled, type="als",
...
set.seed(1)
suppressWarnings({
    model <- softImpute(X_train_scaled, type="als",
                        rank.max=50, lambda=35, maxit=15)
})
